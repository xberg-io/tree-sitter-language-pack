...
// alef:hash:99744e5c15584e7ad624fd75420975d8d9371d4d0333facefd49684bc08c3920
...
impl ByteRange {
    ...
    fn from_json(json_str: String) -> pyo3::PyResult<Self> {
        serde_json::from_str::<tree_sitter_language_pack::ByteRange>(&json_str)
            .map(Into::into)
...
impl DataAttribute {
    ...
    #[staticmethod]
    fn from_json(json_str: String) -> pyo3::PyResult<Self> {
        serde_json::from_str::<tree_sitter_language_pack::DataAttribute>(&json_str)
            .map(Into::into)
            .map_err(|error| pyo3::exceptions::PyValueError::new_err(error.to_string()))
    }
}
...
impl DataNode {
    ...
    #[staticmethod]
    fn from_json(json_str: String) -> pyo3::PyResult<Self> {
        serde_json::from_str::<tree_sitter_language_pack::DataNode>(&json_str)
            .map(Into::into)
            .map_err(|error| pyo3::exceptions::PyValueError::new_err(error.to_string()))
    }
}
...
impl Point {
    ...
    #[staticmethod]
    fn from_json(json_str: String) -> pyo3::PyResult<Self> {
        serde_json::from_str::<tree_sitter_language_pack::Point>(&json_str)
            .map(Into::into)
            .map_err(|error| pyo3::exceptions::PyValueError::new_err(error.to_string()))
    }
}
...
impl DocstringFormat {
    ...
    #[getter]
    fn python_triple_quote(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "PythonTripleQuote" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("PythonTripleQuote") {
    ...
    #[getter]
    fn js_doc(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "JSDoc" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("JSDoc") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn rustdoc(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Rustdoc" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Rustdoc") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn go_doc(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "GoDoc" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("GoDoc") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn java_doc(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "JavaDoc" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("JavaDoc") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn other(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Other" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Other") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
...
impl StructureKind {
    ...
    #[getter]
    fn function(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Function" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Function") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn method(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Method" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Method") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn class(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Class" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Class") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn r#struct(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Struct" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Struct") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn interface(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Interface" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Interface") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn r#enum(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Enum" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Enum") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn module(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Module" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Module") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn r#trait(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Trait" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Trait") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn r#impl(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Impl" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Impl") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn namespace(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Namespace" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Namespace") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
...
impl SymbolKind {
    ...
    #[getter]
    fn variable(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Variable" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Variable") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn constant(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Constant" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Constant") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
    ...
    #[getter]
    fn r#type(&self, py: Python<'_>) -> PyResult<Option<pyo3::Py<pyo3::PyAny>>> {
        let json =
            serde_json::to_value(&self.inner).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
        let payload = match &json {
            serde_json::Value::String(value) if value == "Type" => serde_json::json!({}),
            serde_json::Value::Object(values) => match values.get("Type") {
                Some(value) => value.clone(),
                None => return Ok(None),
            },
            _ => return Ok(None),
...
#[pyfunction]
#[pyo3(signature = ())]
pub fn manifest_groups(py: Python<'_>) -> PyResult<Vec<String>> {
    py.detach(tree_sitter_language_pack::manifest_groups)
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pyo3::create_exception!(_native, ParseTimeoutError, Error);
...
fn error_to_py_err(e: tree_sitter_language_pack::Error) -> pyo3::PyErr {
    ...
    match &e {
        ...
        tree_sitter_language_pack::Error::ParseTimeout { .. } => ParseTimeoutError::new_err(msg),
...
pub fn _native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(manifest_groups, m)?)?;
    ...
    m.add("ParseTimeoutError", m.py().get_type::<ParseTimeoutError>())?;
