...
// alef:hash:92dde3ba41363ecd394cffae222be714380e9eab620e896a1c1277489aad82c7
...
impl DocstringInfo {
    ...
    pub fn get_format(&self) -> DocstringFormat {
        self.format.clone()
    }
...
impl StructureItem {
    ...
    pub fn get_kind(&self) -> StructureKind {
        self.kind.clone()
    }
...
// `DocstringFormat` is EXTERNALLY tagged in the core crate, so serde writes a unit variant as a
...
impl serde::Serialize for DocstringFormat {
        ...
        S: serde::Serializer,
    {
        match self.type_tag.as_str() {
            "PythonTripleQuote" => serializer.serialize_str("PythonTripleQuote"),
            "JSDoc" => serializer.serialize_str("JSDoc"),
            "Rustdoc" => serializer.serialize_str("Rustdoc"),
            "GoDoc" => serializer.serialize_str("GoDoc"),
            "JavaDoc" => serializer.serialize_str("JavaDoc"),
            "Other" => {
                use serde::ser::SerializeMap;
                let mut map = serializer.serialize_map(Some(1))?;
                map.serialize_entry("Other", self.other.as_deref().unwrap_or_default())?;
                map.end()
            }
...
impl<'de> serde::Deserialize<'de> for DocstringFormat {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for AlefExternalVisitor {
            type Value = DocstringFormat;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a string or an object for DocstringFormat")
            }
            ...
            {
                match value {
                    "PythonTripleQuote" => Ok(DocstringFormat {
                        type_tag: "PythonTripleQuote".to_string(),
                        other: None,
                    }),
                    "JSDoc" => Ok(DocstringFormat {
                        type_tag: "JSDoc".to_string(),
                        other: None,
                    }),
                    "Rustdoc" => Ok(DocstringFormat {
                        type_tag: "Rustdoc".to_string(),
                        other: None,
                    }),
                    "GoDoc" => Ok(DocstringFormat {
                        type_tag: "GoDoc".to_string(),
                        other: None,
                    }),
                    "JavaDoc" => Ok(DocstringFormat {
                        type_tag: "JavaDoc".to_string(),
                        other: None,
                    }),
                    "" => Ok(DocstringFormat::default()),
                    value => Err(serde::de::Error::unknown_variant(
                        value,
                        &["PythonTripleQuote", "JSDoc", "Rustdoc", "GoDoc", "JavaDoc", "Other"],
                    )),
                }
            }
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if entries.iter().any(|(key, _)| key == "type") {
                    let mut value = DocstringFormat::default();
                    for (key, entry) in entries {
                        match key.as_str() {
                            "type" => value.type_tag = entry.as_str().unwrap_or_default().to_string(),
                            "other" => value.other = entry.as_str().map(str::to_string),
                            _ => {}
                        }
                    }
                    return Ok(value);
                ...
                match entries.into_iter().next() {
                    Some((key, entry)) if key == "Other" => Ok(DocstringFormat {
                        type_tag: "Other".to_string(),
                        other: entry.as_str().map(str::to_string),
                    }),
                    Some((key, _)) => Err(serde::de::Error::unknown_variant(
                        key.as_str(),
                        &["PythonTripleQuote", "JSDoc", "Rustdoc", "GoDoc", "JavaDoc", "Other"],
                    )),
                    None => Ok(DocstringFormat::default()),
                }
            }
        }
...
impl DocstringFormat {
    ...
    #[php(name = "pythonTripleQuote")]
    pub fn _factory_python_triple_quote() -> Self {
        tree_sitter_language_pack::DocstringFormat::PythonTripleQuote.into()
    }

    #[php(name = "jsDoc")]
    pub fn _factory_js_doc() -> Self {
        tree_sitter_language_pack::DocstringFormat::JSDoc.into()
    }

    #[php(name = "rustdoc")]
    pub fn _factory_rustdoc() -> Self {
        tree_sitter_language_pack::DocstringFormat::Rustdoc.into()
    }

    #[php(name = "goDoc")]
    pub fn _factory_go_doc() -> Self {
        tree_sitter_language_pack::DocstringFormat::GoDoc.into()
    }

    #[php(name = "javaDoc")]
    pub fn _factory_java_doc() -> Self {
        tree_sitter_language_pack::DocstringFormat::JavaDoc.into()
    }

    #[php(name = "other")]
    pub fn _factory_other(other: String) -> Self {
        tree_sitter_language_pack::DocstringFormat::Other(other).into()
    }
    ...
    #[php(getter)]
    pub fn get_other(&self) -> Option<String> {
        self.other.clone()
    }
}
...
// `StructureKind` is EXTERNALLY tagged in the core crate, so serde writes a unit variant as a
// bare string and a data variant as a single-keyed object whose value IS the payload -- never the
// flat `{"type": ...}` object this class stores its state in. A derived impl therefore
// could not read the wire the core type emits, and every value of this type failed to
// deserialize. The flat object form is still accepted so JSON this class previously produced
...
impl serde::Serialize for StructureKind {
    ...
    {
        match self.type_tag.as_str() {
            "Function" => serializer.serialize_str("Function"),
            "Method" => serializer.serialize_str("Method"),
            "Class" => serializer.serialize_str("Class"),
            "Struct" => serializer.serialize_str("Struct"),
            "Interface" => serializer.serialize_str("Interface"),
            "Enum" => serializer.serialize_str("Enum"),
            "Module" => serializer.serialize_str("Module"),
            "Trait" => serializer.serialize_str("Trait"),
            "Impl" => serializer.serialize_str("Impl"),
            "Namespace" => serializer.serialize_str("Namespace"),
            "Other" => {
                use serde::ser::SerializeMap;
                let mut map = serializer.serialize_map(Some(1))?;
                map.serialize_entry("Other", self.other.as_deref().unwrap_or_default())?;
...
impl<'de> serde::Deserialize<'de> for StructureKind {
        ...
        D: serde::Deserializer<'de>,
    {
        struct AlefExternalVisitor;
        impl<'de> serde::de::Visitor<'de> for AlefExternalVisitor {
            type Value = StructureKind;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a string or an object for StructureKind")
            }
            ...
            {
                match value {
                    "Function" => Ok(StructureKind {
                        type_tag: "Function".to_string(),
                        other: None,
                    }),
                    "Method" => Ok(StructureKind {
                        type_tag: "Method".to_string(),
                        other: None,
                    }),
                    "Class" => Ok(StructureKind {
                        type_tag: "Class".to_string(),
                        other: None,
                    }),
                    "Struct" => Ok(StructureKind {
                        type_tag: "Struct".to_string(),
                        other: None,
                    }),
                    "Interface" => Ok(StructureKind {
                        type_tag: "Interface".to_string(),
                        other: None,
                    }),
                    "Enum" => Ok(StructureKind {
                        type_tag: "Enum".to_string(),
                        other: None,
                    }),
                    "Module" => Ok(StructureKind {
                        type_tag: "Module".to_string(),
                        other: None,
                    }),
                    "Trait" => Ok(StructureKind {
                        type_tag: "Trait".to_string(),
                        other: None,
                    }),
                    "Impl" => Ok(StructureKind {
                        type_tag: "Impl".to_string(),
                        other: None,
                    }),
                    "Namespace" => Ok(StructureKind {
                        type_tag: "Namespace".to_string(),
                        other: None,
                    }),
                    "" => Ok(StructureKind::default()),
                    value => Err(serde::de::Error::unknown_variant(
                        value,
                        &[
                            "Function",
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if entries.iter().any(|(key, _)| key == "type") {
                    let mut value = StructureKind::default();
                    for (key, entry) in entries {
                        match key.as_str() {
                            "type" => value.type_tag = entry.as_str().unwrap_or_default().to_string(),
                            "other" => value.other = entry.as_str().map(str::to_string),
                ...
                match entries.into_iter().next() {
                    Some((key, entry)) if key == "Other" => Ok(StructureKind {
                        type_tag: "Other".to_string(),
                        other: entry.as_str().map(str::to_string),
                    }),
                    Some((key, _)) => Err(serde::de::Error::unknown_variant(
...
impl StructureKind {
    ...
    #[php(name = "function")]
    pub fn _factory_function() -> Self {
        tree_sitter_language_pack::StructureKind::Function.into()
    }

    #[php(name = "method")]
    pub fn _factory_method() -> Self {
        tree_sitter_language_pack::StructureKind::Method.into()
    }

    #[php(name = "class")]
    pub fn _factory_class() -> Self {
        tree_sitter_language_pack::StructureKind::Class.into()
    }

    #[php(name = "struct")]
    pub fn _factory_struct() -> Self {
        tree_sitter_language_pack::StructureKind::Struct.into()
    }

    #[php(name = "interface")]
    pub fn _factory_interface() -> Self {
        tree_sitter_language_pack::StructureKind::Interface.into()
    }

    #[php(name = "enum")]
    pub fn _factory_enum() -> Self {
        tree_sitter_language_pack::StructureKind::Enum.into()
    }

    #[php(name = "module")]
    pub fn _factory_module() -> Self {
        tree_sitter_language_pack::StructureKind::Module.into()
    }

    #[php(name = "trait")]
    pub fn _factory_trait() -> Self {
        tree_sitter_language_pack::StructureKind::Trait.into()
    }

    #[php(name = "impl")]
    pub fn _factory_impl() -> Self {
        tree_sitter_language_pack::StructureKind::Impl.into()
    }

    #[php(name = "namespace")]
    pub fn _factory_namespace() -> Self {
        tree_sitter_language_pack::StructureKind::Namespace.into()
    }

    #[php(name = "other")]
    pub fn _factory_other(other: String) -> Self {
        tree_sitter_language_pack::StructureKind::Other(other).into()
    }
...
// `SymbolKind` is EXTERNALLY tagged in the core crate, so serde writes a unit variant as a
// bare string and a data variant as a single-keyed object whose value IS the payload -- never the
// flat `{"type": ...}` object this class stores its state in. A derived impl therefore
// could not read the wire the core type emits, and every value of this type failed to
// deserialize. The flat object form is still accepted so JSON this class previously produced
...
impl serde::Serialize for SymbolKind {
    ...
    {
        match self.type_tag.as_str() {
            "Variable" => serializer.serialize_str("Variable"),
            "Constant" => serializer.serialize_str("Constant"),
            "Function" => serializer.serialize_str("Function"),
            "Class" => serializer.serialize_str("Class"),
            "Type" => serializer.serialize_str("Type"),
            "Interface" => serializer.serialize_str("Interface"),
            "Enum" => serializer.serialize_str("Enum"),
            "Module" => serializer.serialize_str("Module"),
            "Other" => {
...
impl<'de> serde::Deserialize<'de> for SymbolKind {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for AlefExternalVisitor {
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a string or an object for SymbolKind")
            }
            ...
            {
                match value {
                    "Variable" => Ok(SymbolKind {
                        type_tag: "Variable".to_string(),
                        other: None,
                    }),
                    "Constant" => Ok(SymbolKind {
                        type_tag: "Constant".to_string(),
                        other: None,
                    }),
                    "Function" => Ok(SymbolKind {
                        type_tag: "Function".to_string(),
                        other: None,
                    }),
                    "Class" => Ok(SymbolKind {
                    ...
                    "Module" => Ok(SymbolKind {
                        type_tag: "Module".to_string(),
                        other: None,
                    }),
                    "" => Ok(SymbolKind::default()),
                    value => Err(serde::de::Error::unknown_variant(
                        value,
                        &[
                            "Variable",
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if entries.iter().any(|(key, _)| key == "type") {
                    let mut value = SymbolKind::default();
                    for (key, entry) in entries {
                        match key.as_str() {
                            "type" => value.type_tag = entry.as_str().unwrap_or_default().to_string(),
                            "other" => value.other = entry.as_str().map(str::to_string),
                ...
                match entries.into_iter().next() {
                    Some((key, entry)) if key == "Other" => Ok(SymbolKind {
                        type_tag: "Other".to_string(),
                        other: entry.as_str().map(str::to_string),
                    }),
                    Some((key, _)) => Err(serde::de::Error::unknown_variant(
...
impl SymbolKind {
    ...
    pub fn from_json(json: String) -> PhpResult<Self> {
        ...
        if !matches!(
            value.type_tag.as_str(),
            "Variable" | "Constant" | "Function" | "Class" | "Type" | "Interface" | "Enum" | "Module" | "Other"
        ) {
            return Err(PhpException::default(format!(
                "Invalid SymbolKind tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    ...
    #[php(name = "variable")]
    pub fn _factory_variable() -> Self {
        tree_sitter_language_pack::SymbolKind::Variable.into()
    }

    #[php(name = "constant")]
    pub fn _factory_constant() -> Self {
        tree_sitter_language_pack::SymbolKind::Constant.into()
    }

    #[php(name = "function")]
    pub fn _factory_function() -> Self {
        tree_sitter_language_pack::SymbolKind::Function.into()
    }

    #[php(name = "class")]
    pub fn _factory_class() -> Self {
        tree_sitter_language_pack::SymbolKind::Class.into()
    }

    #[php(name = "type")]
    pub fn _factory_type() -> Self {
        tree_sitter_language_pack::SymbolKind::Type.into()
    }

    #[php(name = "interface")]
    pub fn _factory_interface() -> Self {
        tree_sitter_language_pack::SymbolKind::Interface.into()
    }

    #[php(name = "enum")]
    pub fn _factory_enum() -> Self {
        tree_sitter_language_pack::SymbolKind::Enum.into()
    }

    #[php(name = "module")]
    pub fn _factory_module() -> Self {
        tree_sitter_language_pack::SymbolKind::Module.into()
    }

    #[php(name = "other")]
    pub fn _factory_other(other: String) -> Self {
        tree_sitter_language_pack::SymbolKind::Other(other).into()
    }
...
impl From<DocstringFormat> for tree_sitter_language_pack::DocstringFormat {
    fn from(val: DocstringFormat) -> Self {
        match val.type_tag.as_str() {
            ...
            _ => tree_sitter_language_pack::DocstringFormat::default(),
        }
    }
}
...
impl From<StructureKind> for tree_sitter_language_pack::StructureKind {
    fn from(val: StructureKind) -> Self {
        match val.type_tag.as_str() {
            ...
            _ => tree_sitter_language_pack::StructureKind::default(),
        }
    }
}
...
impl From<SymbolKind> for tree_sitter_language_pack::SymbolKind {
    fn from(val: SymbolKind) -> Self {
        match val.type_tag.as_str() {
            ...
            _ => tree_sitter_language_pack::SymbolKind::default(),
        }
    }
}
...
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    ...
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("tree_sitter_language_pack", "1.20.0")
