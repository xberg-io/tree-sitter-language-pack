...
// alef:hash:e24c00ddb6fce7a17b717552c2353551959b4a01aa4f549b76e69908e34b2c6f
...
/// # Example
#[allow(clippy::missing_errors_doc)]
// Wrapper DTO to handle camelCase field mapping from JS
#[derive(Deserialize)]
...
impl From<ProcessConfigInput> for tree_sitter_language_pack::ProcessConfig {
    fn from(input: ProcessConfigInput) -> Self {
        Self {
            language: input.language.unwrap_or_default().into(),
            structure: input.structure.unwrap_or(true),
            imports: input.imports.unwrap_or(true),
            exports: input.exports.unwrap_or(true),
            comments: input.comments.unwrap_or(false),
            docstrings: input.docstrings.unwrap_or(false),
            symbols: input.symbols.unwrap_or(false),
            diagnostics: input.diagnostics.unwrap_or(false),
            chunk_max_size: input.chunk_max_size,
        }
    }
}
...
pub fn process(source: String, config: JsValue) -> Result<WasmProcessResult, JsValue> {
    let config_core: tree_sitter_language_pack::ProcessConfig = if config.is_undefined() {
        ...
        serde_wasm_bindgen::from_value::<ProcessConfigInput>(config)
            .map_err(|e| JsValue::from_str(&e.to_string()))?
            .into()
    };
    let result =
