...
// alef:hash:147c8702e15dad8d50548e4836e093b650120de451825886e8b8293f18527b7c
...
final class ErrorHandlingTest extends TestCase
{
    ...
    {        $config = ProcessConfig::from_json('{}');        $this->expectException(\Exception::class);
    ...
    {        $config = ProcessConfig::from_json(json_encode(["language" => "javascript"]));        $this->expectNotToPerformAssertions();        $result = TreeSitterLanguagePack::process("", $config);
    }
    ...
    {        $config = ProcessConfig::from_json(json_encode(["language" => "haskell"]));        $this->expectNotToPerformAssertions();        $result = TreeSitterLanguagePack::process("{-aaaaaaaaaaaaaa aaaa}\n    {-aaa (aaaaaaaaaa [aaaaaaaaaaaaa aaa", $config);
    }


    /** Parsing invalid syntax should produce a tree with error nodes. */
    public function test_error_handling_invalid_syntax(): void
    {        $config = ProcessConfig::from_json(json_encode(["language" => "javascript"]));        $this->expectNotToPerformAssertions();        $result = TreeSitterLanguagePack::process("function function function @@@ %%%", $config);
    }


    /** Loading a nonexistent language should produce an error. */
    public function test_error_handling_unknown_language(): void
    {        $config = ProcessConfig::from_json(json_encode(["language" => "nonexistent_xyz"]));        $this->expectException(\Exception::class);
    ...
    {        $config = ProcessConfig::from_json(json_encode(["language" => "python"]));        $result = TreeSitterLanguagePack::process("", $config);
