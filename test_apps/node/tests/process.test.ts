...
// alef:hash:14f7395dcc7cee68b322df1f2d62c94062697c273c057be445e8c160a4fcea87
...
describe('process', () => {
  ...
  it('process_with_extractions_map: Process: extractions field accepts a Map of ExtractionPattern and returns a Map of PatternResult — exercises the Java/Python/etc. typed-map surface (#115)', () => {
    const result = process("def alpha():\n    pass\n\ndef beta(x):\n    return x\n", { extractions: { functions: { captureOutput: "Full", query: "(function_definition name: (identifier) @name)" } }, language: "python" });
    expect(result.language.trim()).toBe("python");
    expect(result.extractions.functions.matches.length).toBeGreaterThanOrEqual(2);
