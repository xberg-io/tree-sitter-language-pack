...
// alef:hash:3ab1cf0bdf5dc5baecb3246a08c339dddc68d2f0e5bc63b622a53f7fbb21b124
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef
import { describe, expect, it } from 'vitest';
import { process, detectLanguageFromContent, detectLanguageFromExtension, detectLanguageFromPath, parseString, extractPatterns, treeHasErrorNodes, runQuery } from '@kreuzberg/tree-sitter-language-pack';
...
describe('error-handling', () => {
  ...
  it('error_handling_unknown_language: Loading a nonexistent language should produce an error.', async () => {
    await expect(async () => {
      await parseString("nonexistent_xyz", Buffer.from("", "base64"));
    }).rejects.toThrow();
  });
  ...
  it('parse_empty_language: parse_string() returns error with empty language name', async () => {
    await expect(async () => {
      await parseString("", Buffer.from("x = 1", "base64"));
    }).rejects.toThrow();
  });
