; ~keep: tree-sitter/tree-sitter-ocaml publishes a single shared `queries/` directory at the
; repo root for both grammars/ocaml (implementation) and grammars/interface (.mli). The shared
; highlights query references `(shebang)`, which exists only in the implementation grammar's
; node-types.json — .mli files have no shebang line. This overlay is the vendored file with that
; one alternative removed; everything else, including capture names, is unchanged.
