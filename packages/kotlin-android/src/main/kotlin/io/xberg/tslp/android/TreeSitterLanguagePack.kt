...
object TreeSitterLanguagePack {
     ...
     *
     * Returns the contents of `indents.scm` (used for auto-indentation) as a static string, or
     * `null` if no indents query is bundled for this language.
     */
    fun getIndentsQuery(language: String): String? =
        TreeSitterLanguagePackBridge.nativeGetIndentsQuery(language)

    /**
     * Get the folds query for a language, if bundled.
     *
     * Returns the contents of `folds.scm` (used for code folding) as a static string, or `null` if
     * no folds query is bundled for this language.
     */
    fun getFoldsQuery(language: String): String? =
        TreeSitterLanguagePackBridge.nativeGetFoldsQuery(language)
     ...
     * language into the process registry so a subsequent hot loop only parses.
     *
     * Unlike `download()`, this does not trust in-memory availability — it downloads whenever a
     * grammar is not actually loadable from disk (fixing the case where a known-but-not-downloaded
     * grammar is reported present), then resolves and caches every requested language. Call it
     * once, up front, before a parallel workload.
     *
     * **Errors:**
     *
     * Returns `Error.Download` if a required grammar cannot be fetched, or `Error.LanguageNotFound`
     * if a requested name is unknown.
     */
    fun prefetch(languages: List<String>): Unit =
        TreeSitterLanguagePackBridge.nativePrefetch(mapper.writeValueAsString(languages))
