...
// alef:hash:803b6c0840fe2a84116afe68b5a5fbb8599fafec62198fe71d6c593c6de38800
...
public class LanguageRegistry implements AutoCloseable {
   ...
   *
   * <p>Resolves aliases (e.g., {@code "shell"} -&gt; {@code "bash"}, {@code "makefile"} -&gt;
   * {@code "make"}), then looks up the language in the static table. When the {@code
   * dynamic-loading} feature is enabled, falls back to loading a shared library on demand.
   * {@literal @}throws KreuzbergRsException Returns Error::LanguageNotFound if the name (after
   * alias resolution) does not match any known grammar.
   */
   ...
   *
   * <p>Includes statically compiled languages, dynamically loadable languages (if the {@code
   * dynamic-loading} feature is enabled), and all configured aliases.
   */
   ...
   *
   * <p>Returns {@code true} if the language can be loaded, either from the static table or from a
  ...
  public static LanguageRegistry defaultInstance() throws TreeSitterLanguagePackRsException {
    try (var arena = Arena.ofShared()) {
      ...
      if (handle == null || handle.equals(MemorySegment.NULL)) {
        throw new TreeSitterLanguagePackRsException(
            "defaultInstance returned null", (Throwable) null);
      }
